...
const createBulkSaleLineV2 = async (res, lines, lockingSessionId) => {
  try {
    const linesCreated = await SaleLine.bulkCreate(lines);
    ...
    for (const iterator of linesCreated) {
      let limit = Math.max(1, Math.round((iterator.unitRate || 1) * iterator.quantity));
      
      let sql = `UPDATE card SET saleLineId = :saleLineId, card_isused = 1, locking_session_id = '' 
                 WHERE locking_session_id = :lockId AND productId = :productId AND saleLineId IS NULL`;
      let replacements = {
        saleLineId: iterator.id,
      ...
      if (checkVariant && iterator.colorId !== undefined && iterator.colorId !== null) {
        sql += ` AND colorId = :colorId`;
        replacements.colorId = iterator.colorId;
      }
      if (checkVariant && iterator.sizeId !== undefined && iterator.sizeId !== null) {
        sql += ` AND sizeId = :sizeId`;
        replacements.sizeId = iterator.sizeId;
      }

      sql += ` LIMIT :limit`;

      await db.sequelize.query(sql, { replacements });
      logger.info(`[V2] Linked cards to saleLine ${iterator.id} with limit ${limit}`);
    }
    ...
    logger.error('Error in createBulkSaleLineV2: ' + error);
    const headerId = (lines && lines.length > 0) ? lines[0].headerId : null;
    if (headerId) await saleHeaderService.saleHeaderReversal(headerId);
    throw error;
  }
...
const createBulkSaleLineWithoutResV2 = async (lines, lockingSessionId) => {
  logger.info("==> [V2] Creating sale line bulk with locking id " + lockingSessionId)
  try {
    const linesCreated = await SaleLine.bulkCreate(lines);
    logger.info("===> [V2] Line created len: " + linesCreated.length);
    ...
    for (const iterator of linesCreated) {
      try {
        ...
        await db.sequelize.query(sql, { replacements });
        logger.info(`[V2] Update card saleLineId successfully for line ${iterator.id}`);
      } catch (error) {
        logger.error("[V2] Update card saleLineId fail " + error);
        await fullReversal(linesCreated[0]['id']);
        throw new Error("Card is not updated correctly and inventory amount will not be correctly");
      }
    }
    ...
    await saleHeaderService.saleHeaderReversal(lines[0]['headerId']);
    logger.error('[V2] Error inserting rows:', error);
  }
};

const updateBulkSaleLineV2 = async (lines, lockingSessionId, locationId) => {
  const spfStockVarParam = await spfService.getSPFByCode('STOCK.VAR');
  const checkVariant = spfStockVarParam && spfStockVarParam.value === 'Y';
  ...
  for (const incomingLine of lines) {
    ...
    let limit = Math.max(1, Math.round((incomingLine.unitRate || 1) * incomingLine.quantity));

    let sql = `UPDATE card SET saleLineId = :saleLineId, card_isused = 1, locking_session_id = '' 
               WHERE locking_session_id = :lockId AND productId = :productId AND locationId = :locationId AND saleLineId IS NULL`;
    ...
    if (checkVariant && incomingLine.colorId !== undefined && incomingLine.colorId !== null) {
      sql += ` AND colorId = :colorId`;
      replacements.colorId = incomingLine.colorId;
    }
    if (checkVariant && incomingLine.sizeId !== undefined && incomingLine.sizeId !== null) {
      sql += ` AND sizeId = :sizeId`;
      replacements.sizeId = incomingLine.sizeId;
    }

    sql += ` LIMIT :limit`;

    await db.sequelize.query(sql, { replacements });
    logger.info(`[V2] Updated card links for updated sale line ${incomingLine.id}`);
  }
};
...
module.exports = {
  ...
  updateSaleLine,
  createBulkSaleLineV2,
  updateBulkSaleLineV2,
  createBulkSaleLineWithoutResV2
};